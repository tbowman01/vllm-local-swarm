...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql+asyncpg://langfuse:langfuse123@langfuse-db:5432/auth")
...
async def refresh_token(refresh_token: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if redis_client:
            stored_token = await redis_client.get(f"refresh:{user_id}:{refresh_token[:10]}")
